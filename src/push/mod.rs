...
async fn get_auth_push_token() -> ApiResult<String> {
    ...
    let client_id = format!("installation.{}", push.installation_id);

    let params = [("grant_type", "client_credentials"), ("scope", "api.push"), ("client_id", &client_id), ("client_secret", &push.installation_key)];
...
pub async fn register_push_device(user_uuid: Uuid, device: Device) -> ApiResult<()> {
    ...
    let mut url = push.relay_uri.clone();
    url.path_segments_mut().unwrap().push("push");
    url.path_segments_mut().unwrap().push("register");
...
pub async fn unregister_push_device(uuid: Uuid) -> ApiResult<()> {
    ...
    match get_reqwest_client().delete(url).header(AUTHORIZATION, auth_header).send().await {
        Ok(r) => r,
...
async fn send_to_push_relay(notification_data: Value) {
    ...
    let mut url = push.relay_uri.clone();
    url.path_segments_mut().unwrap().push("push");
    url.path_segments_mut().unwrap().push("send");
