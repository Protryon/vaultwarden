...
fn is_valid_domain(domain: &str) -> bool {
    ...
    if let Err(parse_error) = url::Url::parse(format!("https://{domain}").as_str()) {
        ...
        return false;
    } else if domain.is_empty() || domain.contains("..") || domain.starts_with('.') || domain.starts_with('-') || domain.ends_with('-') {
...
async fn check_domain_blacklist_reason(domain: &str) -> Option<DomainBlacklistReason> {
    ...
    if let Some(blacklist) = &CONFIG.advanced.icon_blacklist_regex {
        // Use the pre-generate Regex stored in a Lazy HashMap if there's one, else generate it.
        let is_match = blacklist.is_match(domain);
...
async fn get_icon(domain: &str) -> Option<(Vec<u8>, String)> {
    let path = format!("{}/{}.png", CONFIG.folders.icon_cache().display(), domain);
    ...
    if CONFIG.settings.disable_icon_download {
        return None;
    }
...
async fn icon_is_negcached(path: &str) -> bool {
    ...
    let expired = file_is_expired(&miss_indicator, CONFIG.advanced.icon_cache_negttl).await;
...
async fn icon_is_expired(path: &str) -> bool {
    let expired = file_is_expired(path, CONFIG.advanced.icon_cache_ttl).await;
    ...
    let resp = match get_page(&ssldomain).or_else(|_| get_page(&httpdomain)).await {
        ...
        Err(e) => {
            ...
            if is_ip.is_err() && domain.matches('.').count() > 1 {
                ...
                let base_domain = format!("{base}.{tld}", tld = domain_parts.next_back().unwrap(), base = domain_parts.next_back().unwrap());
...
async fn save_icon(path: &str, icon: &[u8]) {
    match File::create(path).await {
        ...
        Err(ref e) if e.kind() == std::io::ErrorKind::NotFound => {
            create_dir_all(CONFIG.folders.icon_cache()).await.expect("Error creating icon cache folder");
        }
        Err(e) => {
    ...
    fn set_cookies(&self, cookie_headers: &mut dyn Iterator<Item = &header::HeaderValue>, url: &url::Url) {
        ...
        let cookies = cookie_headers.filter_map(|val| {
            std::str::from_utf8(val.as_bytes())
                ...
                .map(|mut c| {
                    c.set_expires(None);
                    c.set_max_age(Some(Duration::from_secs(2 * 60).try_into().unwrap()));
                    c.into_owned()
                })
                .ok()
        });
